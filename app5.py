...
# --- SABİT LİSTE (Kısaltıldı - Aynen Bırakın) ---
FULL_LIST = [
...
if st.button("Taramayı Başlat"):
    ...
    st.dataframe(pd.DataFrame(uygunlar1), use_container_width=True)
    
    st.subheader(f"2. Temel + Teknik Trend Dönüşü ({len(uygunlar2)} Hisse)")
    st.dataframe(pd.DataFrame(uygunlar2), use_container_width=True)
    
    st.subheader(f"3. Gizli Nakit Devi & Değer Avcısı - Liste 1 ({len(uygunlar3)} Hisse)")
    st.dataframe(pd.DataFrame(uygunlar3), use_container_width=True)
    
    st.subheader(f"4. Kısa Sıkıştırması ve Volatilite - Liste 2 ({len(uygunlar4)} Hisse)")
    st.dataframe(pd.DataFrame(uygunlar4), use_container_width=True)
    
    st.subheader(f"5. Zırhlı Trend Dönüşü - Liste 3 ({len(uygunlar5)} Hisse)")
    st.dataframe(pd.DataFrame(uygunlar5), use_container_width=True)
